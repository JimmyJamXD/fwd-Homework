...
fetch(myRequest)
    ...
    .then(data => {
        ...
        for (product of data) {
            ...
            for (let i = 0; i < product.sizes.length; i++) {
                sizeGroup += `<input type="radio" class="btn-check" autocomplete="off" ${checkedString}>
                                        <label class="btn btn-outline-secondary btn-sm" id="${groupName}${i}">${product.sizes[i]}</label>`
